...
public:
    ...
    using GoalHandleGlobalLocalization = rclcpp_action::ServerGoalHandle<GlobalLocalization>;

    /**
     * Node to obtain the global localization of the robot.
     *
     * Subscribers
     * - /initial_pose_estimate : initial pose estimation marked by te user near the real position of the robot
     *
     * Publisher
...
private:
    ...
    rclcpp_action::CancelResponse handleCancel(const std::shared_ptr<GoalHandleGlobalLocalization> goal_handle);
    void handleAccepted(const std::shared_ptr<GoalHandleGlobalLocalization> goal_handle);
    ...
    /**
     * Publishes the initial pose to nav2
     */
     ...
     *
     * Verifies if the robot is localized by checking if the covariance of the estimated amcl
     * pose is within a set threshold
     */
