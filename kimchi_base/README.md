...
The hardware-software-ros interaction in the `kimchi` project is developed using [ROS 2 Control](https://control.ros.org/master/index.html).

This package:
 - Implements `kimchi`'s [hardware interface](https://control.ros.org/master/doc/ros2_control/hardware_interface/doc/writing_new_hardware_interface.html).
 - Provides a communication with microcontroller:
   - `kimchi_base::MotorDriver` class is in charge of the Serial communication for commanding the motors.
...
The hardware interface accepts some parameters that are passed via the urdf description within the `ros2_control` tag (Check [`kimchi_description`](../kimchi_description/urdf/include/kimchi_control.urdf.xacro))
